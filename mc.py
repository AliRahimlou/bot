            ...
            if current_market_cap <= called_market_cap:
                ...
                while current_market_cap > initial_market_cap * 0.75:
                    if time.time() - start_time > 600:  # 10 minutes
                        ...
                        break
                    print(f"Waiting for market cap to drop for {contract_key} (Current: {current_market_cap}, Initial: {initial_market_cap})")
                    await asyncio.sleep(2)  # Wait 2 sec before rechecking
                    current_market_cap = get_current_market_cap(contract_key)
                else:
                    messages = [f"{contract_key}"]
                    await send_messages(chat_id, messages)
...
# working code and some logic stuff to save
...
api_id = os.getenv('API_ID')
api_hash = os.getenv('API_HASH')
chat_id = '@helenus_trojanbot'  # The chat ID or username of the chat you want to send messages to
...
def get_current_market_cap(contract_id):
    market_cap = get_market_cap_from_gmgn(contract_id)
    if market_cap is None:
        market_cap = get_market_cap_from_geckoterminal(contract_id)
    return market_cap
...
async def _process_contract(contract_key, called_market_cap):
    ...
    if (contract_key, called_market_cap) not in sent_contract_ids:
        ...
        initial_market_cap = current_market_cap
        # if current_market_cap <= called_market_cap:
        # high_percentage_threshold = 0.15
        # low_percentage_threshold = 0.35
        # upper_threshold_value = called_market_cap * (1 + high_percentage_threshold)
        # lower_threshold_value = called_market_cap * (1 - low_percentage_threshold)
        # if lower_threshold_value <= current_market_cap <= upper_threshold_value:
        #     print("Current market cap is within 15% of called market cap.")
            messages = [f"{contract_key}"]
            await send_messages(chat_id, messages)
            with db_lock:
                sent_contract_ids.add((contract_key, called_market_cap))
            ...
            # slippage = 0.03
            # threshold = initial_market_cap * 0.75 * (1 - slippage)
            # while current_market_cap > threshold:     

            while current_market_cap > initial_market_cap * 0.75:
                if current_market_cap > initial_market_cap * 1.60:
                    initial_market_cap = current_market_cap                   
                
                if time.time() - start_time > 500:  # 10 minutes
                    print(f"Contract {contract_key} not satisfied within 10 minutes")
                    with db_lock:
                        sent_contract_ids.add((contract_key, called_market_cap))
                        save_json('sent_contracts.json', sent_contract_ids)
